...
function buildReadmeSheet(workbook, opts) {
    ...
    if (hasGene && exportCfg.sheets.geneAnalysis && exportCfg.geneAnalysis && exportCfg.geneAnalysis.enabled) {
        ...
        if (exportCfg.geneAnalysis.dnmRateTest === false) {
            row('Mutation-rate test (withheld)', 'This workbook contains NO de novo mutation-rate test (Test B: k ~ Poisson(λ = 2·N·Σp), where p is a per-gene per-transmission de novo rate and N the trio count). It is withheld pending review, not because of a known defect. HISTORY, because the reason changed: it was first withheld because λ was built from gnomAD\'s lof.mu/mis.mu/syn.mu, which are a MUTABILITY COVARIATE rather than a rate (summed they predict 0.276 coding de novo per trio against a published ~1.0–1.3, at a class balance of 0.319 vs ~0.168) — that would have pushed ~200 genes past the FDR threshold on a single variant at N=220 where correct rates give 32. That defect is FIXED: λ now uses the Samocha-2014 rates bundled from DeNovoWEST (1.074 per trio, ratio 0.161), and no scale is fitted to the cohort. NOTHING on the Gene Analysis tabs depends on it — those are a gene-count null and are unaffected.',
                'withheld pending review', 'Test B suppressed')
        }
...
function buildDnmRateCategoryTab(workbook, dnm, styles) {
    ...
    if (!any) { r++; ws.addRow(['No category has an observed de novo variant in a gene with a bundled de novo mutation rate (Samocha 2014 via DeNovoWEST) in the current export.']); mergeAcross(r) }
    ws.views = [{state: 'frozen', ySplit: headerRowIdx}]
}
...
function buildDnmRatePerGeneTab(workbook, dnm, styles) {
    ...
    banner(`Per-gene view of the same test as "DNM Rate (gene-set)". One row per (gene, track) with an observed curation-pass de novo variant: k ~ Poisson(λ = 2·N·p), N = ${N} trios${reliable ? '' : ' (PROVISIONAL — no Sample-QC file)'}, p = the gene's per-transmission de novo rate for that track's classes (Samocha 2014 model, bundled from DeNovoWEST). No scale is fitted. The LOEUF / pLI columns are the SECOND axis: λ says how SURPRISING the count is (a big, mutable gene expects more by chance), constraint says whether a real variant there would MATTER. They answer different questions and are deliberately not merged — read them together. "P(X≥k)" is a LIVE Excel formula reproducing the q's p-value.`,
        {italic: true, size: 10, color: {argb: 'FF6B7D8D'}})
