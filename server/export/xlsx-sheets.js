...
function buildReadmeSheet(workbook, opts) {
    ...
    // false under ratePrimary:'mane'. Falls back to a table-agnostic phrase when absent.
    const rtName = rateTable ? rateTable.label : 'the bundled rate table'
    const rtAltName = rateTableAlt ? rateTableAlt.label : null
    ...
    if (hasGene && exportCfg.sheets.geneAnalysis && exportCfg.geneAnalysis && exportCfg.geneAnalysis.enabled) {
        ...
        // Test A's own citations must print whether or not Test B ran. They lived inside the
        // Test B gate, so turning Test B off stripped every reference from the workbook —
        // including the FDR method Test A itself uses. A supplement with no citations is not a
        // supplement.
        row('References (Gene Analysis)', 'Benjamini & Hochberg, J R Stat Soc B 1995;57:289 — the FDR procedure behind every q on these tabs. Gene-set libraries and gene-level annotation sources, with their licences, are listed in the "Data sources & licences" section below; each dimension\'s section header also names its own universe size.')
    ...
    // …and only if at least one Test B TAB actually ships: dnmRateTest controls whether the
    // test is COMPUTED, the per-tab switches whether it is SHOWN. Methods for a test whose
    // every tab was deselected would document a workbook the reader does not have.
    if ((hasGene && exportCfg.sheets.geneAnalysis && exportCfg.geneAnalysis && exportCfg.geneAnalysis.enabled && exportCfg.geneAnalysis.dnmRateTest !== false) && (shDnmSet || shDnmGene)) {
        section('DNM Rate — de novo mutation-rate enrichment (Test B)')
        row('Purpose & scope', 'A SECOND, complementary test (its own tab) asking whether more DE NOVO variants fall in a gene set than the germline mutation rate predicts for a cohort of N trios — the classic de novo enrichment framework. DE-NOVO-ONLY: suppressed unless the data has an `inheritance` column (only `de_novo` variants are counted) and the per-gene de novo rate bundle is present. It runs on GRCh37 too: the rate table is keyed by gene symbol and carries no coordinates (if anything it is GRCh37-native, coming from the DDD study). Only the CONSTRAINT dimension is build-gated, because its terms would otherwise mix gnomAD v4.1 against the v2.1.1 the GRCh37 path uses. The Gene Analysis samples/variants tabs (Test A) are the origin-agnostic clustering test and are unaffected; every variant type (incl. indels) remains represented there.')
        ...
        row('References', `THE RATE MODEL. Samocha et al., Nat Genet 2014;46:944 — the trinucleotide de novo mutation model every rate here comes from. It has no successor: Kaplanis & Samocha et al., Nature 2020;586:757 (DeNovoWEST) applies the same model, and the 2020s literature that tests de novo burden still sources its background from it. "2014" is the model\'s date, not a sign of staleness.
THE PER-GENE TABLE actually used for this export: ${rtName}. ${rateTable && rateTable.id === 'mane' ? 'denovonear (github.com/jeremymcrae/denovonear, MIT) over MANE Select v1.5 (Morales et al., Nature 2022;604:310; public domain).' : 'Kaplanis & Samocha et al., Nature 2020;586:757 + HurlesGroupSanger/DeNovoWEST (MIT).'}${rtAltName ? ` The cross-check λ column is the same model built independently as ${rtAltName} — ` + (rateTableAlt && rateTableAlt.id === 'mane' ? 'denovonear (MIT) over MANE Select v1.5 (Morales et al., Nature 2022;604:310).' : 'Kaplanis & Samocha et al., Nature 2020;586:757 + DeNovoWEST (MIT).') : ''}
gnomAD — Karczewski et al., Nature 2020;581:434 and Chen et al., Nature 2024;625:92 — supplies the CONSTRAINT dimension (LOEUF / pLI) and the LOEUF/pLI columns. It supplies NO de novo rate: gnomAD publishes no per-gene de novo rate, and its lof.mu/mis.mu/syn.mu are a constraint-fitting intermediate, not one (measured: summed they predict 0.276 coding de novo per trio against a published ~1.0–1.3, and mu ÷ the Samocha rate is not even constant across genes, so no rescaling could make them one). Earlier versions of this row credited gnomAD for the rates; that was wrong.
THE TESTS. Benjamini & Hochberg, J R Stat Soc B 1995;57:289 (FDR). Ware et al., Curr Protoc Hum Genet 2015;87:7.25.1 (denovolyzeR) — the reference implementation of the 2014 model, used here as an independent check on the class rates rather than as a dependency. Kobren, Moldovan et al., Nat Commun 2025 (RaMeDiES) — the method behind the "P(X≥k | K)" share test, implemented from the published description; their code (GPL-3) and data files are not used.`)
    }

    // --- DNM Rate tabs — column dictionary ---
    // Every other tab had one; these two did not, and they carry the least familiar columns in
    // the workbook. Described BY ROLE with <tier> as a placeholder rather than the literal header
    // text: tier labels resolve per-cohort (they drop "+frameshift" when no Consequence column
    // makes frameshift countable) and this sheet is built before that is known. Naming roles
    // rather than strings is also the only version that cannot drift out of date.
    if (hasGene && exportCfg.sheets.geneAnalysis && exportCfg.geneAnalysis &&
        exportCfg.geneAnalysis.enabled && exportCfg.geneAnalysis.dnmRateTest !== false) {
        section('DNM Rate tabs — column dictionary')
        row('Category / Gene', 'The unit tested: a gene-set category (gene-set tab) or one gene × one track (per-gene tab). A ✓ beside it marks q<0.05 on the POISSON test — see "✓" below.', 'Test B')
        row('<tier> — e.g. "LoF (…)" / "LoF+missense"', 'The OBSERVED COUNT k of curation-pass de novo variants in this category at that cumulative tier. Tiers are nested: LoF ⊆ LoF+missense. The header names exactly what the tier counts: it says "+frameshift" only when a VEP Consequence column made frameshift countable, and otherwise the tier is SNV-only on BOTH the count and the target.', 'Test B')
        row('# genes / # probands', 'Distinct genes carrying a counted de novo, and distinct probands. READ "# genes" BEFORE ANY ✓: one recurrently-hit gene belongs to many categories and lights up every one, so "# genes = 1" is recurrence, not convergence.', 'Test B')
        row('<tier> p/q', 'The POISSON test: k ~ Poisson(λ = 2·N·Σp) — "more de novo than the germline rate predicts?". The ONLY one of the three that can see an ABSOLUTE excess. Needs N and the rate table\'s absolute scale, and its observed÷expected is inflated in proportion by any cohort-wide artefact.', 'Test B')
        row('<tier> p/q (scale-free)', 'The SYNONYMOUS-CONDITIONED test: k ~ Binomial(k + k_syn, θ), θ = Σp/(Σp+Σp syn) — "skewed toward damage relative to this category\'s OWN synonymous variants?". 2·N cancels out of θ, so it needs no trio count. Gene-set tab only; the per-gene tab\'s banner says why it degenerates at gene level.', 'Test B')
        row('<tier> p/q (share)', 'The COHORT-CONDITIONED test: k | K ~ Binomial(K, π), π = Σp ÷ Σp(exome) — "over-represented among the de novo we ACTUALLY SAW?". Needs neither N nor the absolute scale, so it survives a provisional N. Its observed÷expected cannot be moved by any cohort-wide multiplier — and for the same reason it cannot see a genuine exome-wide excess either. Method: Kobren, Moldovan et al., Nat Commun 2025.', 'Test B')
        row('k (<top tier>) / k syn', 'The worked-derivation block begins here. k = the top tier\'s count (each tier\'s own k is its tier column above). k syn = this category\'s synonymous de novo count — the conditioning statistic of the scale-free test, never a discovery quantity.', 'Test B')
        row('Σp (<tier>) — one per tier', 'The category\'s summed per-transmission de novo rate over its AUTOSOMAL genes that HAVE a rate, for exactly the classes that tier counts. ONE PER TIER, so every printed p is checkable: λ = 2·N·Σp, θ = Σp/(Σp+Σp syn), π = Σp/Σp(exome). Summed over ALL the category\'s genes — not only those with an observed variant.', 'Test B')
        row('Σp syn / θ', 'The same sum for the synonymous class (tier-independent), and θ = Σp/(Σp+Σp syn) as a LIVE formula — so you can see for yourself that neither N nor the absolute scale appears in it.', 'Test B')
        row('λ = 2·N·Σp / P(X≥k)', 'The Poisson expectation and its upper-tail p, both live formulas. The constant 2 = the two parental transmissions at risk per proband. NO scale is fitted to this cohort; the rate table is used as published.', 'Test B')
        row('K (cohort) / π / exp share / P(X≥k | K)', 'The share test\'s derivation. K = the cohort\'s OWN de novo total for that tier\'s classes (the "Observed" banner\'s per-class counts sum to it). π = the category\'s share of the exome-wide target. exp share = K·π. P(X≥k | K) is live. If you want the artefact-proof number read the observed÷expected, NOT the p: both p-values shrink as a cohort grows, which is more data measuring the same share, not an artefact being handled.', 'Test B')
        row('λ (cross-check) / λ ratio', `The SAME k scored against the OTHER bundled rate table — the same Samocha 2014 model built independently on a different transcript set${rtAltName ? ' (' + rtAltName + ')' : ''}. A ratio near 1 says the rate source is not what carries that row. A ratio far from 1 says the two transcript sets disagree about that gene\'s size — a fact about annotation, not about your cohort. "—" means the other table has no rate for that gene, which is "unknown", not "zero". There is deliberately no second p-value and no second FDR family: a rival p would only invite reading whichever is smaller.`, 'Test B')
        row('Genes', 'The symbols behind the count, so a category is auditable at a glance and locus heterogeneity is visible.', 'Test B')
        row('p (rate) / λ = 2·N·p   (per-gene)', 'The single gene\'s per-transmission rate for that track\'s classes, and its Poisson expectation. Per-gene λ is tiny, so power comes from RECURRENCE (≥2 de novo in one gene).', 'Test B')
        row('q / q (share)   (per-gene)', 'Benjamini-Hochberg q within that track, EXOME-WIDE: every autosomal gene with a modelable rate was scanned, not just those with an observed variant. The two are SEPARATE families — questions sharing one correction would be several chances at the same α. Synonymous is the calibrator and carries no discovery q ("cal").', 'Test B')
        row('LOEUF / pLI / Constrained?   (per-gene)', 'A SECOND, orthogonal axis beside the rate. Surprise (λ, p) says a de novo here is unexpected; constraint says a real one would MATTER. Reported side by side and deliberately never merged — one blended score would hide which of the two is carrying a gene.', 'gnomAD constraint')
        row('✓', 'Marks Benjamini-Hochberg q<0.05 on the POISSON test ONLY — never on the scale-free or share tests, which print their p/q undecorated. While N is provisional NO ✓ is awarded at all, because the ✓ is gated on the Poisson and the Poisson is the test a provisional N breaks.', 'Test B')
    }
...
// The VARIANT-level view of Test A. It was called "Gene Analysis (DNMs)", which was simply
// wrong: Test A never looks at inheritance (gene-analysis.js has no de_novo gate — grep it),
// so this tab counts EVERY curation-pass variant, de novo or inherited. The name cost a real
// user a real mistake — they moved to deselect this tab for an inherited cohort, when it is
// precisely the view that serves one. Its sibling dedups to one count per proband; this one
// counts variants. Neither is de novo-specific. Only the DNM Rate tabs (Test B) are.
//
// The qKey/pKey/foldKey below keep their engine names (qDnm/pDnm/foldDnmAll). Those are
// internal identifiers on computeConvergence's output, invisible to any reader, and renaming
// them would ripple through gene-analysis.js and its tests for no reader benefit. The boundary
// is deliberate: user-visible strings are corrected, internal keys are left alone.
const GA_VARIANT_TRACK = {sheetName: 'Gene Analysis (variants)', countKey: 'variants', qKey: 'qDnm', pKey: 'pDnm',
    foldKey: 'foldDnmAll', unit: 'PASS variants', unitShort: 'variants', testLabel: 'variant', conservative: false}
// Back-compat alias: this constant was GA_DNM_TRACK for most of the project's life.
const GA_DNM_TRACK = GA_VARIANT_TRACK
 ...
 * where proband i with dᵢ pass variants hits a category gene by chance with pᵢ = 1−(1−p)^dᵢ.
...
function buildGeneAnalysisTab(workbook, conv, styles, track, derivRefs) {
    ...
    // % base for the count cells: cohort probands (samples) or total pass variants (variants).
    ...
    addBanner(track.conservative
        ? 'The SAMPLE test is the conservative, robust read: its expected accounts for EACH proband\'s per-tier pass-variant burden, so a single hypermutated proband can\'t fake convergence (it dedups to one sample AND is expected to hit). A category kept via ≥2 GENES can still show a ✓ on a count of 1 — that is ONE proband hitting ≥2 genes that share the term (within-proband gene convergence), not recurrence across samples; check the "# genes" column. A gene-count null only approximates the de-novo mutation-rate null, so treat marginal q gently. The companion "Gene Analysis (variants)" tab gives the variant-level view.'
        : 'This VARIANT test is a binomial over pass variants at each tier — de novo AND inherited, since Test A never filters on origin. LESS robust than the samples tab (a hypermutated proband inflates it; it is not deduped by proband). Enrichment is upper-tail only (depletion is not tested). Use "Gene Analysis (samples)" as the headline and this for the variant-level view.',
        {italic: true, size: 10, color: {argb: 'FF6B7D8D'}})
    ...
    addBanner(track.conservative
        ...
        : 'Per-row DERIVATION — printed for EVERY tier (not just pass·ALL), so each tier\'s test is reproducible from this sheet. The VARIANT test is Binomial(n, p): each pass variant independently lands in a category gene with p = the shared "p (prev)" column ("% all genes" as a fraction). Per tier: "k variants" = that tier\'s category pass variants; "n pass variants" = that tier\'s pass variants INSIDE THIS DIMENSION\'S GENE UNIVERSE (tested against its own gated total — not the ALL-tier total, and not the cohort-wide total); "Expected n·p" = the chance mean (live). n IS PER-DIMENSION — each dimension is scored only over the genes its own source classifies, so there is no single cohort n: use the row\'s own "n pass variants (T)" column. "P(X≥k)" is a LIVE Excel formula  =1−BINOMDIST(k−1, n, p, TRUE)  that reproduces that tier\'s "p/q" p-value EXACTLY.',
        {italic: true, size: 10, color: {argb: 'FF6B7D8D'}})
    ...
    for (const c of passCells) {
        ...
        derivHeaders.push(
            track.conservative ? `k probands (${t})` : `k variants (${t})`,
            track.conservative ? `n at-risk (${t})` : `n pass variants (${t})`,
...
module.exports = {
    ...
    GA_SAMPLE_TRACK, GA_VARIANT_TRACK, GA_DNM_TRACK, colLetterOf
}
